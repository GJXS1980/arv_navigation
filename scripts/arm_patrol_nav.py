...
class NavTest():
    def __init__(self):
        ...
        self.get_agv_state = rospy.Publisher('agv_state', Int32, queue_size=5)
        rospy.Subscriber("arm_state", Int32, self.get_arm_state)
    ...
    def arv_move_target(self, target_id):
        ...
        if  self.control_type and target_id == 1:
            self.get_agv_state.publish(1)
            rospy.wait_for_message("arm_state",Int32)
            if self.control_type and target_id+1 < len(self.target_point)+1:
                rospy.loginfo("Move To Next Goal: Goal" + str(target_id+1))
                self.arv_move_target(target_id+1)
        elif self.control_type and target_id+1 < len(self.target_point)+1:
            rospy.loginfo("Move To Next Goal: Goal" + str(target_id+1))
            self.arv_move_target(target_id+1)
        else:
            pass
