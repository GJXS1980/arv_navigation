...
class MoveBaseDoor():
	...
	#连接MQTT服务器
        ...
        self.on_subscribe()
        # 订阅陀螺仪数据作为一直在跑的一个线程
        rospy.Subscriber('/xqserial_server/IMU', Imu, self.flag_data)
	...
	# 订阅mqtt服务器的消息
        ...
        if self.AD_flag:
            self.on_publish("/HG_CAR/CAR_MSG", self.AGV_AD_cmd(), 2)
            time.sleep(3.0)
        # 收到回复时停止发送
        elif self.sub_name == "AD" and  self.sub_action == "loading" and self.sub_feedback == 1:
            self.AD_flag = False
            
        if self.TD_flag:
            self.on_publish("/HG_CAR/CAR_MSG", self.AGV_TD_cmd(), 2)
            time.sleep(3.0)
        # 收到回复时停止发送
        elif self.sub_name == "TD" and  self.sub_action == "unloading" and self.sub_feedback == 1:
            self.TD_flag = False
	...
	# 接收到装货完成回到原点
        ...
        if self.sub_name == "AD" and self.sub_dir == "AGV" and self.sub_action == "loading complete" and self.sub_feedback == 0:
            self.action0 = True
            while(self.sub_feedback == 0):
                # ⻔吊向 AGV 回复收到卸货指令
                self.on_publish("/HG_CAR/CAR_MSG", self.AGV_AD_reply(), 2)

        #   门吊装货完成
        if self.sub_name == "TD" and self.sub_dir == "AGV" and self.sub_action == "unloading complete" and self.sub_feedback == 0:
            self.action2 = True
            while(self.sub_feedback == 0):
                # ⻔吊向 AGV 回复收到卸货指令
                self.on_publish("/HG_CAR/CAR_MSG", self.AGV_TD_reply(), 2)
